...
function AlienLaunchMarker:update(dt)
    ...
    elseif self.spawn_allowed and love.keyboard.wasPressed('space') then

        -- spawn two alliens at the same x coordinate as our main allien,
        -- and with the y coordinate shifted by allien height
        local alien_1 = Alien(self.world, 'round', self.aliens[1].body:getX(), self.aliens[1].body:getY() + 35, 'Player')
        alien_1.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 9.75)
        alien_1.fixture:setRestitution(0.4)
        alien_1.body:setAngularDamping(1)
        
        table.insert(self.aliens, alien_1)
        
        local alien_2 = Alien(self.world, 'round', self.aliens[1].body:getX(), self.aliens[1].body:getY() - 35, 'Player')
        alien_2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10.25)
        alien_2.fixture:setRestitution(0.4)
        alien_2.body:setAngularDamping(1)

        table.insert(self.aliens, alien_2)
    end
end
