...
function Level:update(dt)
    ...
    -- toggle the splitting depending on the collision check
    if self.collided then
        self.launchMarker.spawn_allowed = false
    else
        self.launchMarker.spawn_allowed = true
    end
    ...
    if self.launchMarker.launched then
        -- check for the alliens that stopped moving
        local stopped = 0
        -- check for every allien
        for k, alien in pairs(self.launchMarker.aliens) do
            ...
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                -- increment the stopped aliiens amount and compare it with the total amount of alliens
                stopped = stopped + 1
                if stopped == #self.launchMarker.aliens then
                    -- destroy every allien
