...
-- random shift in a gap value
GAP_SHIFT = math.random(-5,50)
...
function PipePair:init(y)
    ...
    -- change the gap to a gap_shift value so it's random 
    self.pipes = {
        ['upper'] = Pipe('top', self.y),
        ['lower'] = Pipe('bottom', self.y + PIPE_HEIGHT + GAP_HEIGHT + GAP_SHIFT)
    }
