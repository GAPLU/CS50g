...
function BeginGameState:init()
    ...
    self.colors = {}
    while #self.colors < 8 do
        table.insert(self.colors, math.random(18))
    end
    -- spawn a board and place it toward the right
    self.board = Board(VIRTUAL_WIDTH - 272, 16, 1, self.colors)
...
function BeginGameState:enter(def)
    ...
    -- assign the variety accodring to the current lvl
    -- and render the board
    self.board.variety = math.min(6, self.level)
